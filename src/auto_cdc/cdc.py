...
class CDC:
    ...
    def delete_cdc_feed(folder_path: str) -> None:
        ...
        table_name = [part for part in folder_path.strip('/').split('/')[3:] if part!= 'parquet']
        ...
        Utils.rm(folder_path, recurse=True)
...
class HelperFunctions:
    ...
    def __init__(self, spark_dataframe: DataFrame, folder_path: str, keys: list, source_timestamp: datetime, exclude_columns_from_tracking: list | None, vacuum_days: int, recurse: bool):
    ...
    def create_table_name_from_folder_path(self) -> str:
        ...
        table_name = [part for part in self.folder_path.strip('/').split('/')[3:] if part!= 'parquet']
        table_name = '_'.join(table_name)
        cat_schema = '.'.join(cat_schema)
    ...
    def get_target_fields(self) -> list:
        if self.spark.catalog.tableExists(self.target_table_name):
            columns = self.spark.read.table(self.target_table_name).schema.names
        else:
            columns = []
        return columns
    ...
    def is_schema_changed(self) -> bool:
        ...
        if all(source_col.lower() in [tareget_col.lower() for tareget_col in target_fields] for source_col in source_fields):
            return False
        else:
            return True
    ...
    def vacuum_data(self) -> None:
        ...
        delta_table = DeltaTable.forName(self.spark, self.target_table_name)
        last_vacuum_timestamp = self.spark.sql(f"""
    ...
    def get_cdc_versions(self, source_timestamp: datetime) -> DataFrame:
        source_timestamp_str = source_timestamp.strftime('%Y-%m-%d')
        df = self.spark.sql(f"""
                        ...
                        qualify ifnull(lead(source_timestamp) over (order by source_timestamp), source_timestamp) >= '{source_timestamp_str}'
                        order by version
                        """)
        return df.cache()
    
    def get_version_of_df(self, version: int) -> DataFrame:
        return (self.spark.read.option("versionAsOf", version).table(self.target_table_name))
    ...
    def get_last_change_version(self) -> int:
        if self.spark.catalog.tableExists(f'{self.target_table_name}_helper'):
            df = self.spark.sql(f"""select version from {self.target_table_name}_helper""")
            version = df.orderBy("version", ascending=False).limit(1).collect()[0]['version']
            return version + 1
        else:
            return 0
    
    def get_latest_cdc(self, starting_version: int, overwrite_condition: bool=False) -> DataFrame:
        ...
        if overwrite_condition:
            ...
            df = self.spark.sql("""
                    SELECT * EXCEPT (_commit_timestamp),
                        CASE 
                            WHEN _change_type != 'delete' 
                            THEN timestampadd(MILLISECOND, 1, _commit_timestamp)
                            ELSE _commit_timestamp
                        END AS _commit_timestamp
                    FROM _sql_df
                """)

        return df.filter(col("_change_type").isin("update_postimage", "insert", "delete"))
    ...
    def create_cdc_helper_table(self, version: int | None) -> None:
        ...
        spark_dataframe = self.spark.createDataFrame(data, schema)
        if not self.spark.catalog.tableExists(helper_table):
    ...
    def merge_data(self, condition: str | None=None) -> None:
        from delta.tables import DeltaTable
        delta_table = DeltaTable.forName(self.spark, self.target_table_name)
    ...
    def final_cdc_push(self, batch_size: int=10, overwrite_condition: bool=False, lexical_prefix: bool=False) -> None:
        latest_version = self.get_latest_cdc_version()
        last_change_version = self.get_last_change_version()
        change_data_df = self.get_latest_cdc(last_change_version, overwrite_condition)
        file_name = AppFunctions.format_file_datetime('cdc', self.source_timestamp, 'parquet')
        ...
        if not change_data_df.isEmpty():
            if self.recurse:
                ...
                for file in files_to_remove:
                    Utils.rm(file)
            AppFunctions.save_data_to_file(change_data_df, output_path, batch_size=batch_size, lexical_prefix=lexical_prefix)
